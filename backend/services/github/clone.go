...
func CloneRepo(repository models.Repository, branch *github.Branch) (repo *git.Repository, path string, err error) {
	path = utils.ComposeStrings("/tmp/gitops/", repository.FullName, "/", branch.GetName())
	...
	repo, err = git.PlainClone(path, false, &git.CloneOptions{
		URL:      url,
		Progress: os.Stdout,
		ReferenceName: plumbing.NewBranchReferenceName(branch.GetName()),
		SingleBranch: true,
		Depth: 1,
	})
